export const menuPage = {
  "eyebrow": "メニュー",
  "title": "La Table のメニュー",
  "text": "料理、ピザ、バー、ワイン、ソフトドリンク、コーヒー、デザートをひと目で見分けやすくした構成です。",
  "note": "メニューのローカライズ版です。",
  "filters": {
    "buttonLabel": "食の好み",
    "emptyLabel": "条件に合う料理はありません。",
    "clearLabel": "リセット",
    "filterableTabs": [
      "cuisine",
      "pizzas"
    ],
    "options": [
      {
        "id": "vegetarian",
        "label": "ベジタリアン"
      },
      {
        "id": "vegan",
        "label": "ヴィーガン"
      },
      {
        "id": "no-pork",
        "label": "豚肉なし"
      },
      {
        "id": "no-fish",
        "label": "魚なし"
      },
      {
        "id": "no-seafood",
        "label": "魚介なし"
      }
    ],
    "optionLabels": {
      "vegetarian": "ベジタリアン",
      "vegan": "ヴィーガン",
      "no-pork": "豚肉なし",
      "no-fish": "魚なし",
      "no-seafood": "魚介なし"
    }
  },
  "tabs": [
    {
      "id": "cuisine",
      "label": "料理",
      "sections": [
        {
          "id": "sharing",
          "title": "シェア",
          "desktopColumn": 0,
          "items": [
            {
              "name": "マリネしたパプリカ",
              "price": "8 €",
              "filters": [
                "vegan"
              ]
            },
            {
              "name": "パニスとアイオリ",
              "price": "9 €",
              "filters": [
                "vegetarian"
              ]
            },
            {
              "name": "薪火焼きナスのキャビア",
              "price": "8 €",
              "filters": [
                "vegan"
              ]
            },
            {
              "name": "パルマハム",
              "price": "11 €",
              "filters": [
                "pork"
              ]
            }
          ]
        },
        {
          "id": "starters",
          "title": "前菜",
          "desktopColumn": 0,
          "items": [
            {
              "name": "アーティチョークのサラダ",
              "price": "17 €",
              "filters": [
                "vegan"
              ]
            },
            {
              "name": "ブッラータと古来種トマト",
              "price": "19 €",
              "filters": [
                "vegetarian"
              ]
            },
            {
              "name": "キヌアと本日のコンディマン",
              "price": "19 €",
              "filters": [
                "vegan"
              ]
            },
            {
              "name": "衣付きチキンのシーザー",
              "price": "18 €"
            },
            {
              "name": "シオのセビーチェ",
              "price": "18 €",
              "filters": [
                "fish"
              ]
            },
            {
              "name": "軽く焼いたマグロのニース風サラダ仕立て",
              "price": "20 €",
              "filters": [
                "fish"
              ]
            }
          ]
        },
        {
          "id": "mains",
          "title": "メイン",
          "desktopColumn": 1,
          "items": [
            {
              "name": "ムール貝のマリニエール",
              "price": "18 €",
              "filters": [
                "seafood"
              ]
            },
            {
              "name": "ローストしたイワシ 季節のヴィエルジュソース",
              "price": "19 €",
              "filters": [
                "fish"
              ]
            },
            {
              "name": "天然エビのガーリックソテー",
              "price": "28 €",
              "filters": [
                "seafood"
              ]
            },
            {
              "name": "丸ごと真鯛のオーブン焼き",
              "price": "32 €",
              "filters": [
                "fish"
              ]
            },
            {
              "name": "イタリア風牛肉タルタル",
              "price": "19 €"
            },
            {
              "name": "チェダークリームのグルメバーガー",
              "price": "19 €"
            },
            {
              "name": "牛ほほ肉のコンフィ",
              "price": "23 €"
            },
            {
              "name": "豚スペアリブのマリネ",
              "price": "23 €",
              "filters": [
                "pork"
              ]
            },
            {
              "name": "牛肉のタリアータ",
              "price": "25 €"
            },
            {
              "name": "ローストチキン胸肉 ポルチーニソース",
              "price": "25 €"
            },
            {
              "name": "仔牛のミラノ風カツレツ",
              "price": "26 €"
            },
            {
              "name": "ジェノベーゼのリングイネ",
              "price": "20 €",
              "filters": [
                "vegetarian"
              ]
            },
            {
              "name": "アラビアータとストラッチャテッラのリガトーニ",
              "price": "23 €",
              "filters": [
                "vegetarian"
              ]
            },
            {
              "name": "ボンゴレのリングイネ",
              "price": "25 €",
              "filters": [
                "seafood"
              ]
            },
            {
              "name": "トリュフクリームのリガトーニ",
              "price": "32 €",
              "filters": [
                "vegetarian"
              ]
            }
          ]
        },
        {
          "id": "extras",
          "title": "付け合わせ",
          "desktopColumn": 0,
          "items": [
            {
              "name": "フレッシュフライドポテト",
              "price": "5 €",
              "filters": [
                "vegan"
              ]
            },
            {
              "name": "クリーミーなポレンタ",
              "price": "5 €",
              "filters": [
                "vegetarian"
              ]
            },
            {
              "name": "ニース風メスクラン",
              "price": "5 €",
              "filters": [
                "vegan"
              ]
            },
            {
              "name": "南仏野菜",
              "price": "5 €",
              "filters": [
                "vegan"
              ]
            }
          ]
        },
        {
          "id": "kids",
          "title": "お子さまメニュー",
          "desktopColumn": 1,
          "items": [
            {
              "name": "お子さまメニュー",
              "description": "シロップウォーター、チキンテンダーとフライドポテトまたはトマトソースパスタ、アイス1スクープ",
              "price": "12 €"
            }
          ]
        }
      ]
    },
    {
      "id": "pizzas",
      "label": "ピザ",
      "sections": [
        {
          "id": "wood-fired-pizzas",
          "title": "薪窯ピザ",
          "layout": "split-items",
          "items": [
            {
              "name": "マルゲリータ",
              "description": "トマトソース、モッツァレラ、バジル",
              "price": "14 €",
              "filters": [
                "vegetarian"
              ]
            },
            {
              "name": "レーヌ",
              "description": "トマトソース、モッツァレラ、ハム、マッシュルーム",
              "price": "17 €",
              "filters": [
                "pork"
              ]
            },
            {
              "name": "4種のチーズ",
              "description": "トマトソース、モッツァレラ、ルブロション、ロックフォール、山羊チーズ",
              "price": "17 €",
              "filters": [
                "vegetarian"
              ]
            },
            {
              "name": "Cannibale",
              "description": "トマトソース、モッツァレラ、玉ねぎのコンフィ、ひき肉、卵",
              "price": "18 €"
            },
            {
              "name": "ナポリ風",
              "description": "トマトソース、モッツァレラ、アンチョビ、ケッパー",
              "price": "16 €",
              "filters": [
                "fish"
              ]
            },
            {
              "name": "4シーズン",
              "description": "トマトソース、モッツァレラ、マッシュルーム、ナス、ミニトマト、アーティチョーク",
              "price": "18 €",
              "filters": [
                "vegetarian"
              ]
            },
            {
              "name": "プロヴァンス風",
              "description": "トマトソース、モッツァレラ、マッシュルーム、ナス、玉ねぎのコンフィ、パプリカ",
              "price": "18 €",
              "filters": [
                "vegetarian"
              ]
            },
            {
              "name": "スペイン風",
              "description": "トマトソース、モッツァレラ、スピアナータ、パプリカ",
              "price": "18 €",
              "filters": [
                "pork"
              ]
            },
            {
              "name": "地中海風",
              "description": "トマトソース、モッツァレラ、玉ねぎのコンフィ、ケッパー、ツナ",
              "price": "19 €",
              "filters": [
                "fish"
              ]
            },
            {
              "name": "Parma",
              "description": "トマトソース、モッツァレラ、ミニトマト、生ハム、ブッラータ",
              "price": "22 €",
              "filters": [
                "pork"
              ]
            },
            {
              "name": "ノルウェー風",
              "description": "白いベース、モッツァレラ、マリネサーモン、ミニトマト",
              "price": "19 €",
              "filters": [
                "fish"
              ]
            },
            {
              "name": "山羊チーズとハチミツ",
              "description": "白いベース、モッツァレラ、山羊チーズ、くるみ、ハチミツ",
              "price": "17 €",
              "filters": [
                "vegetarian"
              ]
            },
            {
              "name": "Calzone",
              "description": "トマトソース、モッツァレラ、マッシュルーム、ハム、卵",
              "price": "18 €",
              "filters": [
                "pork"
              ]
            },
            {
              "name": "Truffala",
              "description": "トリュフクリーム、モッツァレラ、生ハム、ブッラータ",
              "price": "25 €",
              "filters": [
                "pork"
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "bar",
      "label": "バー",
      "sections": [
        {
          "id": "spritz",
          "title": "Spritz",
          "desktopColumn": 0,
          "items": [
            {
              "name": "Aperol Spritz",
              "description": "Aperol 5 cl、prosecco 10 cl、炭酸水、オレンジ",
              "price": "10 €"
            },
            {
              "name": "Hugo Spritz",
              "description": "St-Germain 5 cl、prosecco 10 cl、炭酸水、レモン、ミント",
              "price": "12 €"
            },
            {
              "name": "Limoncello Spritz",
              "description": "Limoncello 5 cl、prosecco 10 cl、炭酸水、レモン",
              "price": "10 €"
            },
            {
              "name": "Amaretto Spritz",
              "description": "Amaretto 5 cl、prosecco 10 cl、炭酸水、オレンジ",
              "price": "11 €"
            },
            {
              "name": "St-Germain Spritz",
              "description": "St-Germain 5 cl、prosecco 10 cl、炭酸水、レモン",
              "price": "12 €"
            },
            {
              "name": "Martini Spritz",
              "description": "Martini Bianco 5 cl、prosecco 10 cl、炭酸水、レモン",
              "price": "10 €"
            },
            {
              "name": "Campari Spritz",
              "description": "Campari 5 cl、prosecco 10 cl、炭酸水、オレンジ",
              "price": "12 €"
            },
            {
              "name": "赤い果実の Spritz",
              "description": "赤い果実のリキュール 5 cl、prosecco 10 cl、炭酸水",
              "price": "12 €"
            },
            {
              "name": "Rosa Spritz",
              "description": "Sarti 5 cl、prosecco 10 cl、炭酸水、オレンジ",
              "price": "11 €"
            },
            {
              "name": "アーティチョーク Spritz",
              "description": "Cynar 5 cl、prosecco 10 cl、炭酸水",
              "price": "12 €"
            }
          ]
        },
        {
          "id": "cocktails",
          "title": "カクテル",
          "desktopColumn": 0,
          "items": [
            {
              "name": "Negroni",
              "description": "Gin 3 cl、Campari 3 cl、赤ベルモット 3 cl",
              "price": "12 €"
            },
            {
              "name": "Sex on the Beach",
              "description": "Vodka 5 cl、オレンジジュース、クランベリージュース、ピーチリキュール",
              "price": "12 €"
            },
            {
              "name": "Mojito",
              "description": "Rum 5 cl、angostura、ブラウンシュガー、ライム、ミント、炭酸水",
              "price": "12 €"
            },
            {
              "name": "ストロベリー Mojito またはパッションフルーツ Mojito",
              "description": "Rum 5 cl、angostura、ブラウンシュガー、ライム、ミント、炭酸水、いちごまたはパッションフルーツのピューレ",
              "price": "13 €"
            },
            {
              "name": "Paloma",
              "description": "Tequila 5 cl、ライムジュース、グレープフルーツジュース、tonic",
              "price": "13 €"
            },
            {
              "name": "Cosmopolitan",
              "description": "Vodka 5 cl、triple sec、クランベリージュース、ライム",
              "price": "13 €"
            },
            {
              "name": "Mule",
              "description": "Moscow、London、Mexican、Kentucky または Caribbean 5 cl",
              "price": "13 €"
            },
            {
              "name": "Sour",
              "description": "Amaretto、vodka、gin、rum または whisky 5 cl",
              "price": "13 €"
            },
            {
              "name": "Pornstar",
              "description": "Vodka 5 cl、レモン、バニラ、パッションフルーツジュース、prosecco 5 cl",
              "price": "14 €"
            },
            {
              "name": "Pina Colada",
              "description": "Rum 5 cl、ココナッツミルク、パイナップルジュース",
              "price": "14 €"
            },
            {
              "name": "パッションまたはいちごの Daiquiri",
              "description": "Rum 5 cl、きび砂糖、ライム、パッションフルーツまたはいちごのピューレ",
              "price": "14 €"
            },
            {
              "name": "Grand Margarita",
              "description": "Grand Marnier 2 cl、tequila 3 cl、ライム、アガベシロップ",
              "price": "15 €"
            },
            {
              "name": "Espresso Martini",
              "description": "Vodka 5 cl、コーヒーリキュール 2 cl、espresso、バニラシロップ",
              "price": "15 €"
            }
          ]
        },
        {
          "id": "draught-beers",
          "title": "生ビール",
          "desktopColumn": 1,
          "items": [
            {
              "name": "Pietra Bionda",
              "description": "25 cl / 50 cl",
              "price": "4,5 € / 8 €"
            },
            {
              "name": "Fada IPA",
              "description": "25 cl / 50 cl",
              "price": "5 € / 9,5 €"
            }
          ]
        },
        {
          "id": "bottled-beers",
          "title": "瓶ビール（33cl）",
          "desktopColumn": 1,
          "items": [
            {
              "name": "1664 Blanche",
              "price": "7 €"
            },
            {
              "name": "Corona",
              "price": "7 €"
            },
            {
              "name": "Corona 0",
              "price": "7 €"
            },
            {
              "name": "Chill Lemon",
              "price": "7 €"
            },
            {
              "name": "Chouffe",
              "price": "8 €"
            }
          ]
        },
        {
          "id": "aperitifs",
          "title": "アペリティフ",
          "desktopColumn": 1,
          "items": [
            {
              "name": "Martini、Campari、Porto (4 cl)",
              "price": "5 €"
            },
            {
              "name": "Prosecco (14 cl)",
              "price": "6 €"
            },
            {
              "name": "Pastis、Ricard (4 cl)",
              "price": "5 €"
            },
            {
              "name": "シャンパーニュ グラス (14 cl)",
              "price": "10 €"
            },
            {
              "name": "氷入りシャンパーニュ (16 cl)",
              "price": "12 €"
            },
            {
              "name": "Kir (14 cl)",
              "price": "6 €"
            },
            {
              "name": "Kir Royal (14 cl)",
              "price": "12 €"
            }
          ]
        },
        {
          "id": "spirits",
          "title": "スピリッツ",
          "desktopColumn": 1,
          "items": [
            {
              "kind": "section",
              "label": "Whisky & bourbon (4 cl)"
            },
            {
              "name": "William Lawson’s",
              "price": "8 €"
            },
            {
              "name": "Jack Daniel’s, Honey, Fire",
              "price": "10 €"
            },
            {
              "name": "Chivas 12年",
              "price": "13 €"
            },
            {
              "name": "Angels Envy",
              "price": "15 €"
            },
            {
              "name": "Nikka From The Barrel",
              "price": "22 €"
            },
            {
              "kind": "section",
              "label": "Vodka (4 cl)"
            },
            {
              "name": "Eristoff",
              "price": "8 €"
            },
            {
              "name": "Grey Goose",
              "price": "14 €"
            },
            {
              "kind": "section",
              "label": "Tequila (4 cl)"
            },
            {
              "name": "Acayucan",
              "price": "8 €"
            },
            {
              "name": "Patron Silver",
              "price": "15 €"
            },
            {
              "kind": "section",
              "label": "Gin (4 cl)"
            },
            {
              "name": "Bombay Original",
              "price": "8 €"
            },
            {
              "name": "Bombay Sapphire",
              "price": "12 €"
            },
            {
              "name": "Star of Bombay",
              "price": "15 €"
            },
            {
              "kind": "section",
              "label": "Rum (4 cl)"
            },
            {
              "name": "Bacardi Blanc",
              "price": "8 €"
            },
            {
              "name": "Captain Morgan",
              "price": "9 €"
            },
            {
              "name": "Diplomatico",
              "price": "13 €"
            },
            {
              "name": "Bacardi Blanco",
              "price": "16 €"
            },
            {
              "kind": "section",
              "label": "Digestifs (4 cl)"
            },
            {
              "name": "Get 27",
              "price": "8 €"
            },
            {
              "name": "Get 31",
              "price": "9 €"
            },
            {
              "name": "Limoncello",
              "price": "8 €"
            },
            {
              "name": "Amaretto",
              "price": "8 €"
            },
            {
              "name": "Sambuca",
              "price": "8 €"
            },
            {
              "name": "Grappa",
              "price": "9 €"
            },
            {
              "name": "Bailey’s",
              "price": "9 €"
            },
            {
              "name": "Saint-Germain",
              "price": "10 €"
            },
            {
              "name": "Armagnac Laubade",
              "price": "12 €"
            },
            {
              "name": "Baileys",
              "price": "12 €"
            },
            {
              "name": "Hennessy XO",
              "price": "28 €"
            },
            {
              "name": "Chartreuse Verte",
              "price": "15 €"
            },
            {
              "kind": "section",
              "label": "割り材"
            },
            {
              "name": "Soft",
              "price": "+1 €"
            },
            {
              "name": "Red Bull",
              "price": "+2,50 €"
            }
          ]
        }
      ]
    },
    {
      "id": "wines",
      "label": "ワイン",
      "sections": [
        {
          "id": "french-wines",
          "title": "フランスワイン",
          "desktopColumn": 0,
          "items": [
            {
              "kind": "section",
              "label": "白ワイン"
            },
            {
              "name": "Côté Terre, Chardonnay | IGP Languedoc",
              "price": "Verre 6 €"
            },
            {
              "name": "Château Paradis | AOP Coteaux d’Aix",
              "price": "50 cl 19 € • 75 cl 28 €"
            },
            {
              "name": "Chardonnay Les Plans Nobles Ropiteau | Vin de France",
              "price": "75 cl 29 €"
            },
            {
              "name": "Domaine Tariquet « Premières Grives » | IGP Côtes de Gascogne",
              "price": "Verre 7 € • 75 cl 30 €"
            },
            {
              "name": "Domaine Les Héritières | AOP Chablis",
              "price": "Verre 9 € • 75 cl 38 €"
            },
            {
              "name": "Domaine de Maltaverne | AOP Pouilly-Fumé",
              "price": "75 cl 44 €"
            },
            {
              "name": "Minuty Prestige | AOP Côtes de Provence",
              "price": "75 cl 45 €"
            },
            {
              "name": "Domaine St Romble Paul Vattan | AOP Sancerre",
              "price": "75 cl 49 €"
            },
            {
              "kind": "section",
              "label": "ロゼワイン"
            },
            {
              "name": "Côté Mer | IGP Méditerranée",
              "price": "Verre 5 €"
            },
            {
              "name": "Château Paradis Cuvée Essentiel | AOP Coteaux d’Aix",
              "price": "50 cl 19 € • 75 cl 28 €"
            },
            {
              "name": "Tasquier | AOP Côtes de Provence",
              "price": "75 cl 32 €"
            },
            {
              "name": "Château Maïme « Heritage » | AOP Côtes de Provence",
              "price": "75 cl 36 €"
            },
            {
              "name": "Minuty Prestige | AOP Côtes de Provence",
              "price": "Verre 9 € • 75 cl 45 €"
            },
            {
              "name": "Miraval | AOP Côtes de Provence",
              "price": "75 cl 55 €"
            },
            {
              "kind": "section",
              "label": "赤ワイン"
            },
            {
              "name": "Cellier des Chartreux | AOP Côtes du Rhône",
              "price": "Verre 6 €"
            },
            {
              "name": "Château Paradis Cuvée Essentiel | AOP Coteaux d’Aix-en-Provence",
              "price": "50 cl 19 € • 75 cl 28 €"
            },
            {
              "name": "Pinot Noir Les Plans Nobles Ropiteau | Vin de France",
              "price": "Verre 8 € • 75 cl 30 €"
            },
            {
              "name": "Cuvée Perrin | AOP Côtes du Rhône",
              "price": "75 cl 35 €"
            },
            {
              "name": "Les Domaniales | AOP Crozes-Hermitage",
              "price": "75 cl 45 €"
            },
            {
              "name": "La Colombière | AOP Saint-Émilion",
              "price": "75 cl 47 €"
            },
            {
              "name": "Le Seuil de Mazeyres | AOP Pomerol",
              "price": "75 cl 65 €"
            },
            {
              "name": "Jean Claude Boisset | Hautes Côtes de Nuits AOP",
              "price": "75 cl 72 €"
            },
            {
              "name": "Famille Perrin Méridion | AOP Châteauneuf-du-Pape",
              "price": "75 cl 76 €"
            }
          ]
        },
        {
          "id": "world-wines",
          "title": "世界のワイン",
          "desktopColumn": 1,
          "items": [
            {
              "kind": "section",
              "label": "赤 — イタリア"
            },
            {
              "name": "Lambrusco Amabile | I.G.T. Lella",
              "price": "75 cl 28 €"
            },
            {
              "name": "Nero D’Avola Lumà | DOP Sicilia",
              "price": "75 cl 35 €"
            },
            {
              "kind": "section",
              "label": "赤 — スペイン"
            },
            {
              "name": "Riscal 1860 | Castilla y Leon AOC",
              "price": "75 cl 32 €"
            },
            {
              "kind": "section",
              "label": "赤 — アルゼンチン"
            },
            {
              "name": "Clos de los Siete | Valle de Uco – Mendoza",
              "price": "75 cl 39 €"
            },
            {
              "kind": "section",
              "label": "赤 — アメリカ"
            },
            {
              "name": "Buena Vista La Petite Syrah | Vin de Californie",
              "price": "75 cl 38 €"
            },
            {
              "kind": "section",
              "label": "赤 — チリ"
            },
            {
              "name": "Mapu | Valle del Maule",
              "price": "75 cl 30 €"
            },
            {
              "kind": "section",
              "label": "白 — イタリア"
            },
            {
              "name": "Santa Margherita, Pinot Grigio | Valdadige DOC",
              "price": "75 cl 33 €"
            },
            {
              "name": "Etna Bianco DOC | Azienda Tornatore",
              "price": "75 cl 45 €"
            },
            {
              "kind": "section",
              "label": "白 — ニュージーランド"
            },
            {
              "name": "Fusional Sauvignon Blanc | Marlborough",
              "price": "75 cl 35 €"
            }
          ]
        },
        {
          "id": "premium-cellar",
          "title": "プレミアムセラー",
          "desktopColumn": 1,
          "items": [
            {
              "kind": "section",
              "label": "白ワイン"
            },
            {
              "name": "Louis Cheze « Pagus Luminis » | Condrieu AOP",
              "price": "75 cl 85 €"
            },
            {
              "name": "Comte Lafond « Grande Cuvée » | Sancerre AOC",
              "price": "75 cl 89 €"
            },
            {
              "name": "Pernand-Vergelesses | AOC, Jadot",
              "price": "75 cl 94 €"
            },
            {
              "name": "Château Simone | AOC",
              "price": "75 cl 99 €"
            },
            {
              "name": "Chassagne-Montrachet AOP | Guy Amiot et Fils",
              "price": "75 cl 110 €"
            },
            {
              "name": "Meursault AOC | Albert Grivault",
              "price": "75 cl 135 €"
            },
            {
              "kind": "section",
              "label": "赤ワイン"
            },
            {
              "name": "Château Carbonnieux",
              "price": "75 cl 85 €"
            },
            {
              "name": "Château de Pez | Saint-Estèphe AOC",
              "price": "75 cl 94 €"
            },
            {
              "name": "Château Prieuré-Lichine | Margaux AOC",
              "price": "75 cl 110 €"
            },
            {
              "name": "Côte-Rôtie AOP | Domaine du Monteillet « Fortis »",
              "price": "75 cl 140 €"
            },
            {
              "name": "Nuits-Saint-Georges AOC | Domaine Faiveley « Montroziers »",
              "price": "75 cl 145 €"
            },
            {
              "name": "Vosne-Romanée AOC | Domaine Faiveley",
              "price": "75 cl 155 €"
            },
            {
              "name": "Château de Beaucastel | Châteauneuf-du-Pape AOC",
              "price": "75 cl 165 €"
            }
          ]
        },
        {
          "id": "champagnes",
          "title": "シャンパーニュ & スパークリング",
          "desktopColumn": 0,
          "items": [
            {
              "name": "Prosecco | Sensi",
              "price": "Verre 6 € • 75 cl 35 €"
            },
            {
              "kind": "section",
              "label": "シャンパーニュ"
            },
            {
              "name": "Henri Loriot",
              "price": "Verre 12 € • 75 cl 70 €"
            },
            {
              "name": "Billecart-Salmon Brut Réserve",
              "price": "75 cl 90 €"
            },
            {
              "name": "Billecart-Salmon Brut Rosé",
              "price": "75 cl 120 €"
            },
            {
              "name": "Ruinart Brut",
              "price": "75 cl 110 €"
            },
            {
              "name": "Ruinart Blanc de Blancs",
              "price": "75 cl 150 €"
            },
            {
              "name": "Dom Pérignon Brut Vintage",
              "price": "75 cl 300 €"
            }
          ]
        }
      ]
    },
    {
      "id": "softs",
      "label": "ソフト",
      "sections": [
        {
          "id": "cold-drinks",
          "title": "冷たいドリンク",
          "items": [
            {
              "name": "Vittel と San Pellegrino 50 cl",
              "price": "4 €"
            },
            {
              "name": "Vittel と San Pellegrino 100 cl",
              "price": "7 €"
            },
            {
              "name": "シロップウォーター",
              "price": "3 €"
            },
            {
              "name": "Coca、Coca Zero、Schweppes Tonic & Agrumes、Fuze Tea、Sprite、Perrier、Orangina (33 cl)",
              "price": "5 €"
            },
            {
              "name": "Red Bull (25 cl)",
              "price": "6 €"
            },
            {
              "name": "フルーツジュース（ACE、オレンジ、トマト、りんご、パイナップル – 25 cl）",
              "price": "5 €"
            },
            {
              "name": "ピーチまたはラズベリーのアイスティー",
              "price": "6 €"
            },
            {
              "name": "フレッシュジュース（オレンジ、レモン – 25 cl）",
              "price": "6 €"
            }
          ]
        },
        {
          "id": "mocktails",
          "title": "モクテル",
          "items": [
            {
              "name": "Virgin Spritz",
              "price": "9 €"
            },
            {
              "name": "Virgin Mojito",
              "price": "9 €"
            },
            {
              "name": "Virgin Colada",
              "price": "9 €"
            },
            {
              "name": "Virgin Paloma",
              "price": "9 €"
            },
            {
              "name": "Virgin Pornstar",
              "price": "9 €"
            },
            {
              "name": "Virgin Exotique",
              "price": "9 €"
            },
            {
              "name": "Virgin Sex On The Beach",
              "price": "9 €"
            }
          ]
        }
      ]
    },
    {
      "id": "coffee",
      "label": "カフェ",
      "sections": [
        {
          "id": "iced-coffee",
          "title": "アイスコーヒー",
          "desktopColumn": 1,
          "items": [
            {
              "name": "Iced Latte",
              "price": "4,50 €"
            },
            {
              "name": "Iced Caramel Latte",
              "price": "5 €"
            },
            {
              "name": "カフェフラッペ",
              "price": "5 €"
            },
            {
              "name": "Iced Matcha Latte",
              "price": "5,50 €"
            }
          ]
        },
        {
          "id": "hot-drinks",
          "title": "温かいドリンク",
          "layout": "split-items",
          "desktopColumn": 0,
          "items": [
            {
              "name": "エスプレッソ",
              "price": "2 €"
            },
            {
              "name": "デカフェ",
              "price": "2 €"
            },
            {
              "name": "エスプレッソマキアート",
              "price": "2,50 €"
            },
            {
              "name": "アメリカーノ",
              "price": "2,50 €"
            },
            {
              "name": "カプチーノ",
              "price": "3,80 €"
            },
            {
              "name": "クリームコーヒー",
              "price": "3,50 €"
            },
            {
              "name": "ダブルエスプレッソ",
              "price": "4 €"
            },
            {
              "name": "ウィーン風コーヒー",
              "price": "4,50 €"
            },
            {
              "name": "紅茶・ハーブティー",
              "price": "4 €"
            },
            {
              "name": "Latte macchiato",
              "price": "5 €"
            },
            {
              "name": "Irish coffee",
              "price": "12 €"
            },
            {
              "name": "ホットチョコレート",
              "price": "4 €"
            },
            {
              "name": "紅茶・ハーブティー",
              "price": "4 €"
            },
            {
              "name": "ウィーン風チョコレート",
              "price": "5 €"
            },
            {
              "name": "バニララテ",
              "price": "5 €"
            },
            {
              "name": "キャラメルラテ",
              "price": "5 €"
            },
            {
              "name": "ヘーゼルナッツラテ",
              "price": "5 €"
            },
            {
              "name": "Chai Latte",
              "price": "5 €"
            },
            {
              "name": "Matcha Latte",
              "price": "5,50 €"
            }
          ]
        },
        {
          "id": "hot-drink-addons",
          "title": "追加",
          "desktopColumn": 1,
          "items": [
            {
              "name": "植物性ミルク",
              "description": "オーツ",
              "price": "+0,90 €"
            },
            {
              "name": "シロップ",
              "description": "バニラ、キャラメル、ヘーゼルナッツ",
              "price": "+0,50 €"
            },
            {
              "name": "ホイップクリーム",
              "price": "+0,50 €"
            }
          ]
        }
      ]
    },
    {
      "id": "desserts",
      "label": "デザート",
      "sections": [
        {
          "id": "dessert-menu",
          "title": "デザート",
          "items": [
            {
              "name": "チーズプレート",
              "price": "9 €"
            },
            {
              "name": "チョコレートムース",
              "price": "8 €"
            },
            {
              "name": "本日のティラミス",
              "price": "8 €"
            },
            {
              "kind": "section",
              "label": "Palais de la Friandise のデザート"
            },
            {
              "name": "本日のデザート",
              "price": "11 €"
            }
          ]
        },
        {
          "id": "ice-cream",
          "title": "アイスクリーム",
          "items": [
            {
              "kind": "section",
              "label": "手作りアイス"
            },
            {
              "name": "フレーバー",
              "description": "コーヒー、チョコレート、バニラ、ミント、ココナッツ、レモン、いちご、マンゴー、パッションフルーツ、ラズベリー、キャラメル、フィオールディラッテ、パイナップル、ピスタチオ、bananerella、バジル"
            },
            {
              "name": "1スクープ",
              "price": "3 €"
            },
            {
              "name": "2スクープ",
              "price": "5,50 €"
            },
            {
              "name": "3スクープ",
              "price": "8,50 €"
            },
            {
              "kind": "section",
              "label": "アイスデザート"
            },
            {
              "name": "Dame Blanche",
              "description": "バニラアイス、チョコレートソース、ホイップクリーム",
              "price": "9,50 €"
            },
            {
              "name": "コーヒーまたはチョコレート Liégeois",
              "description": "バニラアイス、コーヒーまたはチョコレートアイス、コーヒーまたはホットチョコレート、ホイップクリーム",
              "price": "10 €"
            },
            {
              "name": "エキゾチックサンデー",
              "description": "ココナッツ、マンゴー、パッションフルーツのアイス、ホイップクリーム",
              "price": "10,50 €"
            },
            {
              "name": "赤い果実のサンデー",
              "description": "いちごとラズベリーのアイス、赤い果実のクーリ、ホイップクリーム",
              "price": "11 €"
            },
            {
              "name": "Colonel",
              "description": "レモンアイス、vodka、ホイップクリーム",
              "price": "11 €"
            },
            {
              "name": "After Eight",
              "description": "チョコレートアイス、ミント、Get 27、チョコレート、ホイップクリーム",
              "price": "11 €"
            },
            {
              "name": "Milk shake",
              "description": "アイス + ミルク (33 cl)：ラズベリー、パイナップル、いちご、バナナ、チョコレート、バニラ、マンゴー、パッションフルーツ、ココナッツ、レモン",
              "price": "7 €"
            }
          ]
        }
      ]
    }
  ],
  "winePriceColumnLabels": {
    "glass": "グラス",
    "50cl": "50 cl",
    "75cl": "75 cl"
  }
};
